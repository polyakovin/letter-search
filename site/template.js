...
<div class="hero-art"><div class="art-offset" aria-hidden="true"></div><figure class="hero-picture"><img src="${asset(pic.image)}" alt="${pic.alt}" width="1280" height="955" fetchpriority="high"><figcaption><span>${c.heroCaption}</span><span class="caption-letter">${c.heroLetter}</span></figcaption></figure><div class="sticker" aria-hidden="true">${multiline(c.sticker)}<span>↙</span></div><span class="letter-tile" aria-hidden="true">${pic.letter}<small>1</small></span><svg class="hero-scribble" viewBox="0 0 80 80" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M42 7 35 27M65 23 46 36M71 48 49 46M21 17 24 32"/></svg></div>
</section>
...
<noscript><figure class="nojs-second"><img src="${asset(scenes[locale][1].image)}" alt="${scenes[locale][1].alt}" loading="lazy"><figcaption>${scenes[locale][1].title} · ${scenes[locale][1].letter}</figcaption></figure></noscript>
</div></section>
...
<section class="custom-section section" id="custom"><div class="wrap custom-grid"><div class="custom-art"><figure><img src="${asset('6b06919d-8891-45b4-80e6-f37995adb02c.jpg')}" alt="${locale === 'ru' ? 'Персональная иллюстрация «Гугарк»: знакомый дом и двор с питомцами в фантастическом подводном мире.' : 'Personalized Gugark illustration: a house, garden and pets reimagined as a playful underwater world.'}" width="1200" height="896" loading="lazy"><figcaption>${c.customCaption}</figcaption></figure><div class="custom-sticker" aria-hidden="true">${multiline(c.customBadge)}<span>✷</span></div></div><div class="custom-copy"><p class="eyebrow">04 — ${c.customEyebrow}</p><h2>${multiline(c.customTitle)}</h2><p class="section-intro">${c.customText}</p><ul class="custom-points">${c.customPoints.map((t) => `<li><span aria-hidden="true">✳</span>${t}</li>`).join('')}</ul><a href="${CONTACT}" class="button primary" target="_blank" rel="noopener noreferrer">${c.contact}${arrow}</a><p class="fine contact-note">${c.contactNote}</p></div></div></section>
...
<dialog id="image-dialog" aria-labelledby="dialog-title"><div class="dialog-header"><h2 id="dialog-title">${pic.title}</h2><button id="close-dialog" type="button" class="button outline button-small">${c.close} ×</button></div><p class="fine">${c.zoomHint}</p><img id="zoom-image" alt="${pic.alt}"><div class="dialog-actions"><a id="original-image" href="${asset(pic.image)}" target="_blank" rel="noopener">${locale === 'ru' ? 'Открыть оригинал' : 'Open original'} ↗</a></div></dialog>
</body></html>`;
}
